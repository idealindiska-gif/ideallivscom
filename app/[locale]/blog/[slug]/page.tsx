...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    const url = `${siteConfig.site_domain}/sv/blog/${slug}`;
    const urlEn = `${siteConfig.site_domain}/blog/${slug}`;
...
export default async function SwedishBlogPostPage({ params }: Props) {
  const { locale, slug } = await params;
  ...
  try {
    const [fetchedPost, productsResponse, fetchedRecentPosts] = await Promise.all([
      getPostBySlug(slug),
    ...
    promotionProducts = translateProducts(productsResponse.data, 'sv');
  } catch (error) {
  ...
  const decodedTitle = decodeHtmlEntities(post.title.rendered);
  const currentUrl = `${siteConfig.site_domain}/sv/blog/${slug}`;
    ...
    <div className="min-h-screen bg-background">
            ...
            <div className="relative container mx-auto px-4 md:px-6 lg:px-8 max-w-screen-2xl py-24 md:py-32 lg:py-40">
              ...
              <nav className="mb-6">
                <ol className="flex items-center gap-2 text-sm text-white/80">
                  ...
                  <li>/</li>
                  <li><Link href="/sv/blog" className="hover:text-white">Blogg</Link></li>
                  {categories.length > 0 && (
                    <>
                      <li>/</li>
          ...
          <div className="bg-gradient-to-br from-primary/10 via-background to-secondary/5 container mx-auto px-4 md:px-6 lg:px-8 max-w-screen-2xl py-16 md:py-20">
            <nav className="mb-6">
              <ol className="flex items-center gap-2 text-sm text-muted-foreground">
                <li><Link href="/sv" className="hover:text-primary">Hem</Link></li>
                <li>/</li>
                <li><Link href="/sv/blog" className="hover:text-primary">Blogg</Link></li>
                {categories.length > 0 && (
                  <>
                    <li>/</li>
              ...
              <div className="sticky top-24 space-y-8">
                ...
                {promotionProducts.length > 0 && (
                  <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                    <h3 className="text-xl font-heading font-bold text-primary mb-4">Senaste Erbjudanden</h3>
                    <div className="space-y-4">
                      {promotionProducts.map((product) => (
                        <Link
                          key={product.id}
                          href={`/sv/product/${product.slug}`}
                ...
                {recentPosts && recentPosts.length > 0 && (
                  <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                    <h3 className="text-xl font-heading font-bold text-primary mb-4">Senaste Artiklarna</h3>
                    <div className="space-y-4">
                      {recentPosts.filter((recentPost: any) => recentPost.slug !== slug).slice(0, 4).map((recentPost: any) => (
                        <Link
                          key={recentPost.id}
                          href={`/sv/blog/${recentPost.slug}`}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(wordPressArticleSchema(post, `${siteConfig.site_domain}/sv`))
        }}
      />
      <script
        type="application/ld+json"
            ...
            postBreadcrumbs(
              {
                ...
                } : { name: 'Blogg', slug: 'blog' },
              },
              `${siteConfig.site_domain}/sv`
            )
          ))
        }}
      />
